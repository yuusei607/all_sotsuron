...
if __name__ == "__main__":
    with Controller.open(
        ...
        #     )   
        stm = FociSTM(
            foci=[np.array([w/2, h, 200.0]), np.array([w/2, h + 0.0, 200.0])],
            config=1.0 * Hz,
        )
        ...
        plus_point = np.array([0.0, -1.0, 0.0])
        while True:
            plus_point[1] += 1.0
            stm = FociSTM(
                foci=[np.array([w/2, h, 200.0]), np.array([w/2, h + 0.0, 200.0]) + plus_point],
                config=1.0 * Hz,
            )
            autd.send((m,stm))
            print(f'現在の幅：{plus_point[1]}')
            _ = input()
